...
<style scoped>
.course-card { --card-ink: var(--ink, var(--home-v2-ink, #10223f)); --card-deep: var(--deep, var(--home-v2-deep, var(--secondary-color, #28366c))); --card-accent: var(--teal, var(--home-v2-blue, var(--primary-color, #28366c))); --card-line: var(--line, var(--home-v2-line, #dce3e5)); --card-heading: var(--heading, var(--home-v2-heading, inherit)); display:flex; min-width:0; flex-direction:column; overflow:hidden; border:1px solid var(--card-line); background:#fff; color:var(--card-ink); box-shadow:0 10px 28px -30px #12313966; transform-style:preserve-3d; transform-origin:center; will-change:transform; transition:border-color .25s ease, transform .25s ease, box-shadow .25s ease; }
...
.course-cover { position:relative; height:300px; overflow:hidden; padding:20px; background:#d8e9e5; transform:translateZ(1px); }
...
.level-pill { position:absolute; right:20px; bottom:18px; padding:7px 11px; background:#ffffffe0; color:var(--card-ink); font-size:10px; font-weight:800; }
...
.course-content p { display:-webkit-box; min-height:68px; margin:0; overflow:hidden; color:#6b7e82; font-size:13px; line-height:1.75; -webkit-box-orient:vertical; -webkit-line-clamp:3; }
...
.course-footer { display:grid; grid-template-columns:1fr auto 38px; align-items:center; gap:12px; margin-top:auto; padding-top:18px; border-top:1px solid var(--card-line); font-size:12px; font-weight:800; }.course-footer>b { color:var(--card-deep); font:900 13px var(--card-heading); }
.course-arrow { display:grid; width:38px; height:38px; place-items:center; background:var(--card-ink); color:#fff; font-size:18px; transition:background-color .2s ease,transform .2s ease; }.course-card:hover .course-arrow { background:var(--card-accent); transform:translateX(-2px); }
