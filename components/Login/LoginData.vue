...
<template>
    <div class="login-container">
        ...
        <div class="login-form">
            ...
            <p>{{ $t('تعطي صفحة تسجيل الدخول الأولوية لأمن المستخدم، وتقدم تجربة سلسة تضمن الوصول السريع والمريح مجموعةمن فوائد النظام.') }}</p>

            <div class="inputs">
                <div class="phone">
                    <div class="login-input">
                        <input type="text" placeholder="رقم الهاتف" v-model="Credential" @keydown="handleKeyPress">
                        <callIcon class="login-call-icon" />
                    </div>
                    <div class="phone-code" v-if="UserSettingStore?.setting?.country_code_required">

                        <Select :defaultValue="{ dial_code: `${UserSettingStore?.setting?.country_code}` }"
                ...
                <div class="login-input">
                    <input type="password" placeholder="كلمه المرور" v-model="LoginPassword" @keydown="handleKeyPress">
                    <LockIcon class="login-call-icon" />
                </div>

                <div class="login-or">
                    <nuxt-link to="/login/resetpassword">
                        <p>{{ $t('هل نسيت كلمه المرور..؟') }}</p>
                    </nuxt-link>
