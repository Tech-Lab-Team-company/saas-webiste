...
<template>
  <div :class="['list-search-card', showMap ? 'row' : 'col']">
    <img
      v-if="listing?.status === ProjectStatusEnum.Inactive"
      class="closed"
    ...
    <div class="list-search-card-img">
      ...
      <NuxtImg
        ...
        loading="lazy"
      />
      <span class="sponsor" v-if="listing?.isSponsored">
        <svg
          width="16"
          height="16"
          viewBox="0 0 16 16"
    ...
    <NuxtLink
      ...
      class="list-search-card-info"
      :class="[
        listing?.status === ProjectStatusEnum.Inactive ? 'disabled' : '',
      ]"
    >
      <div class="flex gap-1">
        <p
          class="list-search-card-type"
          v-for="category in listing?.categories"
        >
          {{ wordSlice(category?.title, 15) }}
        </p>
      </div>
      <h6 class="list-search-card-title">
        {{ listing?.name }}
      </h6>
      ...
      <p
        class="list-search-card-description"
        v-html="wordSlice(listing?.description, 100)"
      ></p>
