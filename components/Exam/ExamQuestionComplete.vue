...
const IncreaseIndex = () => {
  if (
    questions.value[QuestionIndex.value].answers &&
...
<template>
  <div class="question-container">
    ...
    <div class="question-title">
      ...
      <p v-if="questions[QuestionIndex]?.hint">
      ...
      <div class="questions" v-if="questions[QuestionIndex].answers">
        <div class="question">
          <label
            for="answer1"
            class="correct"
            @click="SelectedAnswer[QuestionIndex] = 'answer1'"
            :class="
              SelectedAnswer[QuestionIndex] == 'answer1' ? `selected` : ``
            "
            >{{ questions[QuestionIndex]?.answers?.[0] ?? "" }}</label
          >
          <input
            class="answer"
        ...
        <div class="question">
          <label
            for="answer2"
            class="correct"
            @click="SelectedAnswer[QuestionIndex] = 'answer2'"
            :class="
              SelectedAnswer[QuestionIndex] == 'answer2' ? `selected` : ``
            "
            >{{ questions[QuestionIndex]?.answers?.[1] ?? "" }}</label
          >
          <input
            class="answer"
            v-model="selected[QuestionIndex]"
