    ...
    <div class="profile-sidebar-container">
        <div class="person-data">
            <div class="profile-image-container">
                <img src="../../public/images/person4.jpg" alt="person">
                <EditImageIcon class="edit-icon" />
            </div>
            <p class="person-name">أحمد حوام</p>
            <p class="person-stage">طالب ثانوي</p>
        </div>

        <ul class="profile-options">
            <!-- @click="$router.push('/dashboard/profile')" -->
            <li class="profile-option" @click="UpdateSidebar('profile')" :class="SelectedOption == 'profile' ? `active`: ``">
                <p>الملف الشخصي</p>
                <SettingsIcon class="profile-icon" />
            </li>
            <li class="profile-option" @click="UpdateSidebar('security')" :class="SelectedOption == 'security' ? `active`: ``">
                <p>الامان</p>
                <key class="profile-icon" />
            </li>
            <li class="profile-option " @click="UpdateSidebar('courses')" :class="SelectedOption == 'courses' ? `active`: ``">
                <p>كورساتي</p>
                <CoursesNote class="profile-icon" />
            </li>
            <li class="profile-option" @click="UpdateSidebar('quetionsbank')" :class="SelectedOption == 'quetionsbank' ? `active`: ``">
                <p>بنك اسئلتي</p>
                <QuestionBank class="profile-icon" />
            </li>
            <li class="profile-option" @click="UpdateSidebar('logout')" :class="SelectedOption == 'logout' ? `active`: ``">
                <p>تسجيل الخروج</p>
